...
def get_draft_tokens(lst_token, previous_tokens, token_map, vocab_size, order, GUESS_SET_SIZE, GUESS_SIZE): 
    ...
    for o in order:
        if o == "W":
            ...
            # if order.index("W") < order.index("L") and len(tokens) < 7:
                # previous_token_num = len(tokens)
            added_tokens = get_db(previous_tokens, vocab_size, GUESS_SIZE, GUESS_SET_SIZE)
            # if previous_token_num > 0:
                # update_token_map(token_map, lst_token, None, added_tokens, 5, 7, 7, LRU=False)
            tokens += added_tokens
            guess_source = 2
        else:
    ...
    while True:
        ...
        if DO_WM:
            if past_tokens[LEVEL - 2] is not None and GUESS_SET_SIZE > 0:  
                ###############NOT ENTER CURRENTLY
                # if input_ids.shape[1] - init_len > PREVIOUS_TOKEN_NUM:
                #     from IPython import embed; embed(); exit(0)
