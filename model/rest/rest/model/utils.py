...
def get_nucleus_posterior_mask(logits, candidates, temperature, top_p, top_k):
...
def evaluate_posterior(
    logits, candidates, temperature, top_k=50, top_p=0.9
):
    """
    ...
    elif top_p > 0 or top_k > 0:
        assert top_p < 1.0, "top_p should between 0 and 1"
        posterior_mask = get_nucleus_posterior_mask(logits, candidates, temperature, top_p, top_k)
        candidates_accept_length = (torch.cumprod(posterior_mask, dim=1)).sum(dim=1)
        accept_length = candidates_accept_length.max()
        # Choose the best candidate
        if accept_length == 0:
